---
name: codex
description: Invoke Codex CLI (GPT-5.4) for web research, second opinions, code review,
  or autonomous terminal tasks. Use when user says "use codex", "ask codex", "codex review",
  "codex research", "search the web for", or invokes /codex. Also auto-invoked by
  subagent-driven-development at phase boundaries.
---
...
**Auto-invocation uses `compare` mode** with a prompt constructed from:
...
```bash
bash ~/.claude/hooks/codex-delegate.sh review "" --cwd "$(pwd)"
```

**User:** "get codex's opinion on this architecture"
```bash
bash ~/.claude/hooks/codex-delegate.sh compare "Review the architecture — is the service layer properly separated?" --cwd "$(pwd)"
```
